...
known. This is particularly useful for deciding is a predictor,
...
\dcode{na.action} is the missing value action; \dcode{na.omit}
should also work.
...
for $\log(\vn{Hg})$.  The function recognizes that
...
specified. The documentation for these functions given more
