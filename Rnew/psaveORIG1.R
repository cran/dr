...
dr.temp.psave=function(dat) {
    ...
    wt.cov <- function(z) cov(z) * (dim(z)[1]-1)/dim(z)[1]
    temp.save=function(z,y,h) {
        ...
        for (j in 1:h) {
            ...
            else IminusC=diag(rep(1,p))-wt.cov(z[ind,]) 
    ...
    directions=apply(SigmaPool%^%(-1/2)%*%evectors,2,function(x) x/sqrt(sum(x^2)))
    ...
    return(list(directions=data.frame(directions),test=test,
    coordinate.test=coordinate.test,M=M))
}

dr.fit.psave1 <-function(object,numdir=4,nslices=NULL,...){ 
    dat=list()
    dat$X=dr.x(object)
    dat$Y=dr.y(object)
    dat$G=object$group
